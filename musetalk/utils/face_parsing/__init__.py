...
class FaceParsing():
    ...
    def _postprocess_parsing(self, parsing, mode="raw"):
        """Apply the same morphological post-processing as ``__call__`` to a
        single parsing array (uint8 [512, 512]). Used by :meth:`batch_call`.
        """
        if mode == "neck":
            parsing[np.isin(parsing, [1, 11, 12, 13, 14])] = 255
            parsing[np.where(parsing != 255)] = 0
    ...
    def batch_call(self, images, size=(512, 512), mode="raw"):
        """Batched version of :meth:`__call__`.

        ``images`` is a list of PIL.Image (or paths). Runs the BiSeNet
        forward pass **once** on the stacked input ``[N, 3, 512, 512]``,
        then applies the per-frame morphological post-processing in a
        Python loop on small 512x512 uint8 arrays.

        Returns a list of PIL.Image masks (mode "L"), one per input image.
        """
        if not images:
            return []
        ...
        for image in images:
            if isinstance(image, str):
                image = Image.open(image)
            pil_images.append(image.resize(size, Image.BILINEAR))

        with torch.no_grad():
            batch_tensor = torch.stack([self.preprocess(img) for img in pil_images])
            if torch.cuda.is_available():
                batch_tensor = batch_tensor.cuda()
            out = self.net(batch_tensor)[0]               # [N, num_classes, 512, 512]
            parsing_batch = out.cpu().numpy().argmax(1)   # [N, 512, 512]

        results = []
        for parsing in parsing_batch:
            parsing = self._postprocess_parsing(parsing, mode=mode)
            results.append(Image.fromarray(parsing.astype(np.uint8)))
        return results
